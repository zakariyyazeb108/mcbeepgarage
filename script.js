...
function getPullCount() {
  ...
  const stored = JSON.parse(localStorage.getItem("pullData")) || {};
...
function incrementPullCount() {
  ...
  localStorage.setItem("pullData", JSON.stringify(stored));
}
...
saveGame();
  ...
  // Update money and message
  message.innerHTML = `You have ${MAX_DAILY_PULLS - getPullCount()} pulls left today.`;
  moneyDisplay.textContent = `💰 Money: $${money.toLocaleString()}`;
...
devSubmit.addEventListener("click", () => {
  ...
  if (code === "299279") {
    ...
    makeCodePopup.classList.remove("hidden"); // 👈 THIS LINE
